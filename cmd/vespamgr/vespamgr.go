...
func (v *VespaMgr) QueryXappConf(appmgrUrl string) (appConfig []byte, err error) {
	...
	for i := 0; i < v.appmgrRetry; i++ {
		app.Logger.Info("Getting xApp config: %s", appmgrUrl)
...
func (v *VespaMgr) SubscribeXappNotif(appmgrUrl string) {
	targetUrl := fmt.Sprintf("http://service-ricplt-vespamgr-http.ricplt.svc.cluster.local:8080/%s", v.appmgrNotifUrl)
